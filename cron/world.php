...
if ($err) {
    ...
    $sql_last_inserted_id = "SELECT `id` FROM `world` ORDER BY `id` desc LIMIT 1";
    $result = $conn->query($sql_last_inserted_id);
    if ($result->num_rows > 0) {
        // Done successfully
        $last_inserted_id = $result->fetch_assoc()['id'];
    } else {
        $subject_insert = "World Table Fetching last ID error ->  " . date("Y-m-d H:i:s");
    ...
    if (sizeof($res) > 15) {
        ...
        if ($i >= 15) {
            $sql_truncate = "Delete from world where id <= " . $last_inserted_id;
