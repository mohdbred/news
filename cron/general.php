...
curl_setopt($curl, CURLOPT_URL, "https://api.nytimes.com/svc/topstories/v2/home.json" . "?" . http_build_query($query)
);
...
if ($err) {
    ...
    $sql_last_inserted_id = "SELECT `id` FROM `general` ORDER BY `id` desc LIMIT 1";
    $result = $conn->query($sql_last_inserted_id);
    if ($result->num_rows > 0) {
        // Done successfully
        $last_inserted_id = $result->fetch_assoc()['id'];
    } else {
        $subject_insert = "General Table Fetching last ID error ->  " . date("Y-m-d H:i:s");
    ...
    if (sizeof($res) > 15) {
        ...
        foreach ($res as $key => $value) {
            ...
            if (!empty($value['title']) && !empty($value['abstract']) && !empty($value['url']) && !empty($value['multimedia'])) {
                ...
                $sql = "INSERT INTO general (data) VALUES ('" . $test . "')";
                if ($conn->query($sql) === TRUE) {
                    // Done successfully
                    $i++;
                } else {
                    $subject_insert = "General Table Insertion error " . date("Y-m-d H:i:s");
        ...
        if ($i >= 15) {
            $sql_truncate = "Delete from general where id <= " . $last_inserted_id;

            if ($conn->query($sql_truncate) === TRUE) {
                // Done successfully
            } else {
                $subject = "General Table truncation error " . date("Y-m-d H:i:s");
        ...
        // Catching error 
        $subject_count = "Data from general API is below 15 ->  " . date("Y-m-d H:i:s");
        $body_count = "General API is not giving enough data: " . sizeof($res);
