...
curl_setopt($curl, CURLOPT_URL, "https://api.nytimes.com/svc/topstories/v2/politics.json" . "?" . http_build_query($query)
);
...
if ($err) {
    ...
    $sql_last_inserted_id = "SELECT `id` FROM `politics` ORDER BY `id` desc LIMIT 1";
    $result = $conn->query($sql_last_inserted_id);
    if ($result->num_rows > 0) {
        // Done successfully
        $last_inserted_id = $result->fetch_assoc()['id'];
    } else {
        $subject_insert = "Politics Table Fetching last ID error ->  " . date("Y-m-d H:i:s");
    ...
    if (sizeof($res) > 5) {
        ...
        foreach ($res as $key => $value) {
            ...
            if (!empty($value['title']) && !empty($value['abstract']) && !empty($value['url']) && !empty($value['multimedia'])) {
                ...
                $sql = "INSERT INTO politics (data) VALUES ('" . $test . "')";
                if ($conn->query($sql) === TRUE) {
                    // Done successfully
                    $i++;
                } else {
                    $subject_insert = "Politics Table Insertion error " . date("Y-m-d H:i:s");
        ...
        if ($i >= 15) {
            $sql_truncate = "Delete from politics where id <= " . $last_inserted_id;

            if ($conn->query($sql_truncate) === TRUE) {
                // Done successfully
            } else {
                $subject = "Politics Table truncation error " . date("Y-m-d H:i:s");
        ...
        // Catching error 
        $subject_count = "Data from Politics API is below 15 ->  " . date("Y-m-d H:i:s");
        $body_count = "Politics API is not giving enough data: " . sizeof($res);
